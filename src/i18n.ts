const translations: Record<string, Record<string, string>> = {
  tr: {
    'nav.projects': 'Projeler',
    'nav.skills': 'Nitelikler',
    'nav.about': 'Hakkımda',
    'nav.contact': 'İletişim',
    'hero.student': 'Öğrenci',
    'hero.email': 'E-posta gönder',
    'hero.about1': 'Selam, ben Eren. 16 yaşındayım ve <a href="https://www.ademceylantk.com.tr/" target="_blank" rel="noopener">Özel Adem Ceylan Final Teknik Koleji</a>\'nde Bilişim Teknolojileri öğrencisiyim.',
    'hero.about2': 'Yazılıma olan ilgim aslında biraz <em>\'tersten\'</em> başladı. Tasarım yapmayı çok seviyorum ve yapay zeka araçlarıyla çok hızlı sonuçlar alabildiğimi fark edince bir süre <em>\'vibecoding\'</em> yaparak projeler geliştirdim. Ancak bu süreç bana çok önemli bir şey öğretti: Eğer bir teknolojinin arkasında ne dönüp bittiğini gerçekten biliyorsanız, yapay zekayı bir araç olarak çok daha güçlü ve verimli kullanabiliyorsunuz.',
    'hero.about3': 'İşte bu maceram, beni işin mutfağını öğrenmeye itti. Şu an bir yandan yapay zeka ile üretim hızımı korurken, bir yandan da web tasarımı ve mobil uygulama geliştirme süreçlerinde kendimi derinleştiriyorum. Amacım, tasarım estetiğini teknik bilgiyle birleştirip fark yaratan projeler üretmek.',
    'hero.about4': 'Nelerle uğraştığımı merak edersen, <a href="https://github.com/403fx" target="_blank" rel="noopener">GitHub</a> sayfama göz atabilirsin!',
    'currentlyWorking.title': 'Şu an üzerinde çalışıyorum',
    'currentlyWorking.active': 'Aktif',
    'currentlyWorking.e_cenaze': 'Arkadaşlarımla birlikte bu uygulama üzerinde çalışıyorum. Takım çalışması başta zordu ama artık üstesinden gelebiliyoruz.',
    'currentlyWorking.studyMate': 'Öğrenciler için ders çalışma arkadaşı uygulaması.',
    'currentlyWorking.photos': 'Fotoğraflar',
    'skills.title': 'Nitelikler',
    'skills.filter.all': 'Tümü',
    'skills.hint': 'İkonların üzerine gelerek veya tıklayarak detayları görebilirsiniz. Kenarlık rengi türü, opaklık ise yetkinlik seviyesini gösterir.',
    'skills.cisco.desc': 'Ağ yapılandırması ve Cisco cihazları konusunda %90 seviyesinde kullanabiliyorum.',
    'skills.python.desc': 'Temel seviyede Python biliyorum.',
    'skills.linux.desc': 'Linux işletim sistemini kullanabiliyorum.',
    'skills.vscode.desc': 'VS Code editörünü etkin şekilde kullanabiliyorum.',
    'skills.macos.desc': 'macOS işletim sistemini aktif olarak kullanabiliyorum.',
    'skills.nmap.desc': 'Temel seviyede kullanabiliyor, çıktılarını anlıyorum.',
    'skills.prompt.desc': 'Yapay zeka modellerine etkili promptlar yazabiliyorum.',
    'skills.cursor.desc': 'Cursor AI editörünü %100 kullanabiliyorum.',
    'skills.git.desc': 'Git versiyon kontrol sistemini kullanabiliyorum.',
    'skills.html.desc': 'Temel HTML yapısını anlayabiliyorum, baktığımda yapıyı çözebiliyorum.',
    'skills.category.network': 'Ağ',
    'skills.category.language': 'Dil',
    'skills.category.system': 'Sistem',
    'skills.category.tool': 'Araç',
    'projects.title': 'Projeler',
    'project.autoSummarizer': 'Yapay zeka destekli metin özetleme aracı',
    'project.cv': 'Online CV / Portfolyo',
    'project.loadIt': 'Fitness takip uygulaması',
    'project.basicSpoofer': 'Discord ban atlama aracı',
    'project.studyMate': 'Öğrenciler için ders çalışma arkadaşı uygulaması',
    'project.toDoList': 'JS, CSS, HTML öğrenme projesi',
    'project.code': 'Kod',
    'project.live': 'Canlı',
    'contact.title': 'İletişim',
    'footer.text': '© 2026 Eren · Tutkuyla yapıldı',
    'toast.title': 'Başarım Kazanıldı',
    'toast.sub': 'Siteyi ziyaret ettiğin için teşekkürler :)',
  },
  en: {
    'nav.projects': 'Projects',
    'nav.skills': 'Skills',
    'nav.about': 'About',
    'nav.contact': 'Contact',
    'hero.student': 'Student',
    'hero.email': 'Click to send an E-mail',
    'hero.about1': 'Hi, I\'m Eren. I\'m a 16-year-old IT student at <a href="https://www.ademceylantk.com.tr/" target="_blank" rel="noopener">Özel Adem Ceylan Final Teknik Koleji</a>.',
    'hero.about2': 'My journey into software started a bit <em>\'backwards.\'</em> I\'ve always had a passion for design, and when I saw how quickly I could bring ideas to life using AI, I dove into <em>\'vibecoding.\'</em> However, that process taught me a vital lesson: to truly harness the power of AI, you need to understand what\'s happening under the hood.',
    'hero.about3': 'Now, I\'m bridging that gap by mastering the fundamentals of web and mobile development. My goal is to blend design aesthetics with technical expertise to build standout digital products.',
    'hero.about4': 'If you\'re curious about what I\'ve been working on, feel free to check out my <a href="https://github.com/403fx" target="_blank" rel="noopener">GitHub</a>!',
    'currentlyWorking.title': 'Currently working on',
    'currentlyWorking.active': 'Active',
    'currentlyWorking.e_cenaze': 'Currently working on this app with my friends. Working as a team was hard at first but now we can handle it.',
    'currentlyWorking.studyMate': 'Study companion application for students.',
    'currentlyWorking.photos': 'Photos',
    'skills.title': 'Skills',
    'skills.filter.all': 'All',
    'skills.hint': 'Hover or click on the icons to see more details. Border color represents type, opacity represents proficiency level.',
    'skills.cisco.desc': 'I can use Cisco at about 90% proficiency for network configuration and devices.',
    'skills.python.desc': 'Basic level Python knowledge.',
    'skills.linux.desc': 'I can use Linux operating system.',
    'skills.vscode.desc': 'I use VS Code editor effectively.',
    'skills.macos.desc': 'I actively use macOS as my operating system.',
    'skills.nmap.desc': 'Basic level usage, can read and understand outputs.',
    'skills.prompt.desc': 'I can write effective prompts for AI models.',
    'skills.cursor.desc': 'I use Cursor AI editor at 100%.',
    'skills.git.desc': 'I can use Git version control system.',
    'skills.html.desc': 'I understand basic HTML structure and can figure it out when I look at it.',
    'skills.category.network': 'Network',
    'skills.category.language': 'Language',
    'skills.category.system': 'System',
    'skills.category.tool': 'Tool',
    'projects.title': 'Projects',
    'project.autoSummarizer': 'AI-powered text summarization tool',
    'project.cv': 'Online CV / Portfolio',
    'project.loadIt': 'Fitness tracking application',
    'project.basicSpoofer': 'Discord ban bypass utility',
    'project.studyMate': 'Study companion application for students',
    'project.toDoList': 'Trying to learn JS, CSS, HTML',
    'project.code': 'Code',
    'project.live': 'Live',
    'contact.title': 'Contact',
    'footer.text': '© 2026 Eren · Built with passion',
    'toast.title': 'Achievement Unlocked',
    'toast.sub': 'Thank you for visiting my site :)',
  },
}

const STORAGE_KEY = 'eren-lang'

export type Lang = 'tr' | 'en'

export function getStoredLang(): Lang {
  const stored = localStorage.getItem(STORAGE_KEY)
  return (stored === 'en' || stored === 'tr') ? stored : 'tr'
}

export function setStoredLang(lang: Lang): void {
  localStorage.setItem(STORAGE_KEY, lang)
}

export function t(key: string, lang: Lang): string {
  return translations[lang]?.[key] ?? translations.en[key] ?? key
}

export function applyTranslations(lang: Lang): void {
  document.documentElement.lang = lang === 'tr' ? 'tr' : 'en'

  document.querySelectorAll('[data-i18n]').forEach((el) => {
    const key = el.getAttribute('data-i18n')
    if (key) {
      el.textContent = t(key, lang)
    }
  })

  document.querySelectorAll('[data-i18n-html]').forEach((el) => {
    const key = el.getAttribute('data-i18n-html')
    if (key) {
      el.innerHTML = t(key, lang)
    }
  })

  document.querySelectorAll('.lang-btn').forEach((btn) => {
    if (btn.getAttribute('data-lang') === lang) {
      btn.classList.add('active')
    } else {
      btn.classList.remove('active')
    }
  })
}
